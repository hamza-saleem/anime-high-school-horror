...
public class FPSRigidBodyWalker : MonoBehaviour {
	...
	private float stepDist;//forward distance to detect steps in front of player
...
//		Gizmos.DrawRay(eyePos, moveDirection.normalized);
//		Gizmos.DrawSphere(transform.position - upVec * (transform.position.y - capsule.bounds.min.y - 0.4f), 0.1f);
//		Gizmos.color = Color.green;
//		Gizmos.DrawSphere(myTransform.position - upVec * (myTransform.position.y - capsule.bounds.min.y - maxStepHeight) + (checkDirection.normalized * stepDist), 0.1f);
//		Gizmos.DrawSphere(stepHit.point, 0.1f);
//		Gizmos.DrawSphere(stepHit2.point, 0.1f);
//		Gizmos.DrawSphere(stepHit3.point, 0.1f);
	}
...
//		if(Mathf.Abs(InputComponent.moveY) > 0.15f || Mathf.Abs(InputComponent.moveX) > 0.15f){
			...
			if(jumping){
				...
				if(allowJumping){
					...
					if (InputComponent.jumpPress
                        ...
                        //move player backward slightly if walking/running forward 
                        //to prevent getting stuck while jumping against object
                        if(InputComponent.forwardHold){
                            myTransform.position = myTransform.position - (myTransform.forward *  0.1f);
                        }
	...
	void FixedUpdate(){
		...
		if(Time.timeScale > 0){//allow pausing by setting timescale to 0
			...
			if(FPSPlayerComponent.hitPoints > 0.0f){
			...
			if (grounded) {
				...
				if(!crouched){
					if (Physics.Raycast(stepPos, checkDirection.normalized, out stepHit, stepDist, clipMask.value, QueryTriggerInteraction.Ignore)//lower step detection raycast
						...
						&& !stepHit.rigidbody){
						//upper step detection raycast to check for free space above step (make sure lower raycast is not detecting a high vertical wall)
			...
			if((grounded || climbing || swimming || ((airTime + 0.45f) > Time.time)) && FPSPlayerComponent.hitPoints > 0.0f && !FPSPlayerComponent.restarting){
