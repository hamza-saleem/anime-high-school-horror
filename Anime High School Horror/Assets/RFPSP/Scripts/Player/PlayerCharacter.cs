...
public class PlayerCharacter : MonoBehaviour {
	...
	[Tooltip("Additional mesh objects to display with character.")]
	public GameObject[] unattatchedObjs;
    private GameObject[] unattatchedObjs2;
	...
	void Start () {
        ...
        unattatchedObjs2 = fpBodyObj.GetComponent<PlayerCharacter>().unattatchedObjs;
		...
		if(anim.isInitialized && fpBodyAnim.isInitialized && Time.timeScale > 0.0f){
			...
			if(!CameraControlComponent.thirdPersonActive && displayVisibleBody){
				...
				if(tpswitchTime + 0.001 < Time.time || walkerComponent.sprintActive){
                    ...
                    foreach (GameObject obj in unattatchedObjs2) {
					    obj.SetActive (true);
				    }
